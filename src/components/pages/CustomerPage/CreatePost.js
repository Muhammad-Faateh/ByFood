...
const CreatePost = (props) => {
  ...
  useEffect(() => {
    if (
      !(
        UserService.isLoggedIn() && UserService.GetLoggedInUser().role == "User"
      )
    ) {
      history.push("/");
    } else {
      setUser(UserService.GetLoggedInUser());
      setChar(UserService.GetLoggedInUser().name);
    }
  }, []);
        ...
        <Paper elevation={3} style={{ width: "50rem", padding: "1rem" }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={11} md={11}>
              <span style={{ fontSize: "1.5rem" }}>{getUser.name}</span>
            </Grid>
